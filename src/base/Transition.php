...
class Transition extends WorkflowBaseObject implements TransitionInterface
{
	...
	public function __construct($config = [])
	{
		if ( ! empty($config['start'])) {
			...
			if ( ! $this->_startStatus instanceof StatusInterface ) {
				throw new WorkflowException('Start status object must implement jinowom\workflow\base\StatusInterface');
			}
		} else {
		...
		if ( ! empty($config['end'])) {
			...
			if ( ! $this->_endStatus instanceof StatusInterface) {
				throw new WorkflowException('End status object must implement jinowom\workflow\base\StatusInterface');
			}
	 ...
	 * @see \jinowom\workflow\base\WorkflowBaseObject::getId()
	 */
	...
	/**
	 * @see \jinowom\workflow\base\TransitionInterface::getEndStatus()
	 */
	...
	/**
	 * @see \jinowom\workflow\base\TransitionInterface::getStartStatus()
	 */
