...
abstract class WorkflowBaseObject extends BaseObject
{
	...
	public function __construct($config = [])
	{
		...
		if( array_key_exists('source', $config) ) {
			...
			if( ! $this->_source instanceof IWorkflowSource){
				throw new InvalidConfigException('The "source" property must implement interface jinowom\workflow\source\IWorkflowSource');
			}
			unset($config['source']);
		}
	 ...
	 * @throws \jinowom\workflow\base\WorkflowException
	 ...
	 * 
	 * @return \jinowom\workflow\source\IWorkflowSource the source instance or null if no 
